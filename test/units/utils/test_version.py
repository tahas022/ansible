...
def test_from_loose_version_coverage():
    assert SemanticVersion.from_loose_version(
        LooseVersion("1.2.3-alpha+build")
    ) == SemanticVersion("1.2.3-alpha+build")
    assert SemanticVersion.from_loose_version(
        LooseVersion("1.2.3+build-alpha")
    ) == SemanticVersion("1.2.3+build-alpha")
    assert SemanticVersion.from_loose_version(
        LooseVersion("1.2.3-alpha")
    ) == SemanticVersion("1.2.3-alpha")
    assert SemanticVersion.from_loose_version(
        LooseVersion("1.2.3+build")
    ) == SemanticVersion("1.2.3+build")
    assert SemanticVersion.from_loose_version(LooseVersion("1")) == SemanticVersion(
        "1.0.0"
    )  # Empty version in LooseVersion
    with pytest.raises(ValueError):
        SemanticVersion.from_loose_version(
            LooseVersion("1.a.2-beta")
        )  # Non-int component before extra
...
def test_cmp_coverage():
    ...
    assert SemanticVersion("1.0.0+build") == SemanticVersion(
        "1.0.0"
