...
class TestCliBuildVaultIds(unittest.TestCase):
    ...
    # New test cases added based on your request:

    def test_empty_vault_ids_no_flags(self):
        # Test with an empty list of vault ids and no flags
        res = cli.CLI.build_vault_ids([], auto_prompt=False, ask_vault_pass=False)
        self.assertEqual(res, [])

    def test_empty_vault_ids_with_auto_prompt(self):
        # Test with an empty list and auto_prompt set to True
        res = cli.CLI.build_vault_ids([], auto_prompt=True)
        self.assertEqual(res, ["default@prompt_ask_vault_pass"])

    def test_invalid_vault_password_file(self):
        # Test with an invalid vault password file
        res = cli.CLI.build_vault_ids([], vault_password_files=["/invalid/path"])
        self.assertEqual(res, ["default@/invalid/path"])

    def test_mixed_vault_ids_with_auto_prompt_and_ask_vault_pass(self):
        # Test with a mix of vault ids and both flags enabled
        res = cli.CLI.build_vault_ids(
            ["valid@prompt", "invalid@prompt"],
            vault_password_files=["/some-file"],
            ask_vault_pass=True,
            auto_prompt=True,
        )
        self.assertEqual(
